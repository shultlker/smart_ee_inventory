...
| 全量回归 | `pytest tests/`（69 用例） | API / 服务 / 网关 / 配置 |
...
| 场景 | 行为 |
|------|------|
| 出库/入库 | **待确认弹窗**，须填使用人/项目；取消可回滚 pending |
...
- **测试体系**：pytest **69** 用例 + 冒烟门禁（**26 例 / 0.6s**）+ RFID 模拟器  
...
- **质量门禁**：冒烟 26 例 / 0.6s · 全量 69 例 · verify_seed 校验演示库  
...
覆盖路径：`test_gateway/` · `test_epc_binding` · `test_seed_data` · `test_inventory_edit` · 标签 bind/rebind · `test_inventory_edit_api`
...
```
61 passed, 7 skipped, 2 warnings in ~3s
（69 collected；7 条 API 测试在无种子时 skip）
```
...
> 「核心链路有 **26 项冒烟测试不到 1 秒跑完**，协议与看门狗另有 **30 项专项用例**；演示库 `verify_seed` 保证 BIN-TEST 与三枚标签绑定一致。」
